...
# Plot
plot(0, 0, xlim=c(0,5), ylim=c(0,5), type="n", xlab="X", ylab="Y", asp=1)
arrows(0, 0, i[1], i[2], col="red", length=0.1)
arrows(0, 0, j[1], j[2], col="blue", length=0.1)
arrows(0, 0, a[1], a[2], col="black", length=0.1)

legend("topright", legend=c("i", "j", "a"), col=c("red", "blue", "black"), lty=1)
```
...
```{r}
T <- matrix(c(5,-2,4,1),byrow=FALSE, nrow=2)
...
plot(0, 0, xlim=c(-5,5), ylim=c(-5,5), type="n", xlab="X", ylab="Y", asp=1)
arrows(0, 0, i[1], i[2], col="red", length=0.1)
arrows(0, 0, j[1], j[2], col="blue", length=0.1)
arrows(0, 0, ti[1], ti[2], col="green", length=0.1)
arrows(0, 0, tj[1], tj[2], col="purple", length=0.1)
legend("topright", legend=c("i", "j", "new i", "new j"), col=c("red", "blue", "green", "purple"), lty=1)
```
...
arrows(0, 0, i[1], i[2], col="red", length=0.1)
arrows(0, 0, j[1], j[2], col="blue", length=0.1)
arrows(0,0, v[1], v[2], col = "magenta", length=0.1)
arrows(0,0, nv[1], nv[2], col = "orange", length=0.1)
arrows(0, 0, ti[1], ti[2], col="green", length=0.1)
arrows(0, 0, tj[1], tj[2], col="purple", length=0.1)
legend("topright", legend=c("i", "j", "new i", "new j", "v", "new v"), col=c("red", "blue", "green", "purple", "magenta", "orange"), lty=1)
```
...
```{r}
T <- matrix(c(-2,0,0,2), byrow=TRUE, ncol=2)
ni <- T %*% i
nj <- T %*% j 

plot(0,0, xlim = c(-5,5), ylim = c(-5,5), asp=1, type="n", xlab="X", ylab="Y")
arrows(0, 0, i[1], i[2], col="red", length=0.1)
arrows(0, 0, j[1], j[2], col="blue", length=0.1)
arrows(0,0, ni[1], ni[2], col = "green", length=0.1)
arrows(0,0, nj[1], nj[2], col = "purple", length=0.1)
legend("topright", legend=c("i", "j", "new i", "new j"), col=c("red", "blue", "green", "purple"), lty=1)
...
```{r}
T <- matrix(c(3,2,0,0), byrow=TRUE, ncol=2)
ni <- T %*% i
nj <- T %*% j 
...
```{r}
# Define vectors
v <- c(9, -1)  # Substantially longer vector
w <- c(6, -3)

# Compute projection
proj_scalar <- sum(v * w) / sum(w * w)
proj <- proj_scalar * w

# Plot setup
plot(0, 0, type = "n", xlim = c(-1, 7), ylim = c(-5, 3),
     xlab = "", ylab = "", asp = 1, axes = TRUE)
grid()

# Draw span of w as a full line
slope <- w[2] / w[1]
abline(0, slope, col = "blue", lwd = 1)

# Draw v
arrows(0, 0, v[1], v[2], col = "pink", lwd = 5, length = 0.1)

#Draw w 
arrows(0, 0, w[1], w[2], col="green", lwd=5, length=0.1)

# Draw proj(v) on w
arrows(0, 0, proj[1], proj[2], col = "magenta", lwd = 2, length = 0.1)

# Draw perpendicular connection
segments(v[1], v[2], proj[1], proj[2], col = "purple", lty = 3)

# Add legend
legend("topright", legend = c("v", "w", "proj v on w"),
       col = c("pink", "green", "magenta"), lwd = 2, bty = "n")

title(main="Projection of v on w")

# print("Length of projected v:")
# sqrt(sum(proj^2))
# 
# print("Length of w:")
# sqrt(sum(w^2))
